...
### Approximate Split Finding and Weighted Quantile Sketch in XGBoost

We first look at a basic greddy approach for finding best split point.
...
How do we find these candidate split points? We need to assign each feature value a weight to make candidates distribute evenly on the data. For each observation, XGBoost used its second-order derivative (i.e. $$h_i$$) s weight. Formally, let multi-set $$D_k = {(x_{1k},h_1), (x_{2k}, h_2)...(x_{nk}, h_{n})}$$ denote k-th feature value and its corresponding second-order gradient, the rank function is $$r_k(z) = \frac{1}{\sum_{(x,k) \in D_k}h}\sum_{(x,k) \in D_k, x < z}h$$.
...
<!-- -->
```
-   XGBoost supports parallel computing during the process of finding the best split point in building a single tree. Then the information gain calculation for each feature can be done in parallel. Moreover, You can tune the parameter nthread to control the number of cores used in training XGBoost model.
